...
If you need to create your `annotations.csv` file, please follow the example `create_annotations_table()`.
See `help("create_annotations_table")` for further details on how to save the table to modify it.


```{r eval=FALSE}
annotations <- create_annotations_table(samples_directory, "*.mea")
```
...
ket1 <- findPeaks(ket1, noise_level = 2)
peak_list_ket1 <- peaks(ket1)

ket1 <- getGCIMSSample(dataset, "Ketones1")
plt <- plotRaw(ket1)
plt <- add_peaklist_rect(plt, peak_list_ket1)
plt
```
...
```{r}
findPeaks(dataset, noise_level = 2)
peak_list <- peaks(dataset)
```
You can get any other sample if you like, plot it and plot the peaks on top:
...
Or plot all the peaks from all the dataset together, overlayed on a single reference sample:
