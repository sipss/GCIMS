...
It is possible to customize the `SNR.Th`, the `ridgeLength` and any other sensitive
peak detection parameters using the [defaultPeakDetectionCWTParams()] function.

To do that, first create the peakDetectionCWTParams object:
...
All those are parameters we can pass to [MassSpecWavelet::peakDetectionCWT()] and
subsequently to [MassSpecWavelet::identifyMajorPeaks()]. We can add some other
parameter to customize the peak detection algorithm:

```{r}
# Filter peaks lower than 1000 in amplitude:
peakDetectionCWTParams$peakThr <- 1000
```

When using a custom `peakDetectionCWTParams` you do not need to provide the
`peakwidth_range_xunits`, since that is already accounted for in the prepared
wavelets provided in the scales, make sure to set it to `NULL` to avoid
further confusion:
...
ch2 <- findPeaks(
  ...
  verbose = FALSE,
  length_in_xunits = 3,
  peakwidth_range_xunits = NULL,  # provided already in peakDetectionCWTParams
  peakDetectionCWTParams = peakDetectionCWTParams,
  extension_factor = 0,
  iou_overlap_threshold = 0.2,
  debug = TRUE
)
peak_list_ch2 <- peaks(ch2)
plot(ch2) + 
  geom_vline(xintercept = peak_list_ch2$apex, color = "red", linetype = "dashed") +
  labs(caption = "Here we only see one peak, because we have excluded peaks with amplitude < 1000")
```


We can apply `findPeaks` to one whole sample.
