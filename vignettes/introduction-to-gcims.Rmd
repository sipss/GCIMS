...
```{r}
#VISUALIZO
gcims_view_sample(alignd ,sample_num = 1, rt_range = NULL, dt_range = NULL, transform = TRUE)
gcims_plot_spec(alignd, samples = 1:3, rt_value =NULL, dt_range = c(7.2,10), colorby = "Name")
gcims_plot_chrom(alignd, samples = 1:3, dt_value =NULL , rt_range =  NULL, colorby = "Name")
```
...
correction_type <- optimize_align_tr(dir_in = alignd,  samples = 1:3)

# ALIGN IN RETENTION TIME
align_tr(dir_in = alignd, dir_out = alignr, samples = 1:3, 1)
```
...
```{r}
# PLOT SPECTRA
gcims_plot_spec(dir_in = alignr, samples = 1:3, rt_value = NULL, dt_range = NULL, colorby = "Name")
```
...
```{r}
# PLOT CHROMATOGRAMS
gcims_plot_chrom(dir_in = alignr, samples = 1:3, dt_value = NULL, rt_range = NULL, colorby = "Name")
```
...
gcims_view_sample(dir_in = alignr, sample_num = 1, rt_range = NULL, dt_range = NULL)
```
...
```{r}
# ROIs DETECTION
peak_list <- gcims_rois_selection(dir_in = alignr, dir_out = roispp, samples = 1:3, noise_level = 3)
head(peak_list)
...
```{r}
peak_list_foms <- gcims_figures_of_merit(dir_in = bslnr, dir_out = fom, samples = (1:3), peak_list = tt$peak_list_clustered)
head(peak_list_foms)
```
...
```{r}
peak_table <- build_peak_table(peak_list_clustered, aggregate_conflicting_peaks = max)
...
```{r}
peak_table_imputed <- gcims_missing_imputation(dir_in = bslnr, peak_table_with_na, peak_list_foms)
