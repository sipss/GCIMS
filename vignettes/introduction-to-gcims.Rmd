...
Data visualization functions are useful to understand data structure. The package provides three main data visualization functions to perform this task: `gcims_view_sample()`, `gcims_plot_spec()`, and `gcims_plot_chrom()`. Let's use function `gcims_view_sample()` to inspect sample 1. 
...
Data is aligned in drift and retention time axes applying the function `gcims_align_data()`. The correction in drift time axis is multiplicative, that is, a multiplicative correction factor is computed for each of the samples so that the position of the Reactant Ion Peak (RIP) in spectra is the same across samples. The correction in retention time axis is achieved using the Parametric Time Warping (PTW) algorithm. 
...
Chemical information is GCIMS data is found in peaks. To characterize a sample, one must detect all their present peaks. Also, one must know the correspondence of these peaks across samples. These two tasks are carried out by the functions `gcims_select_rois()` and `group_peak_list()`, respectively. 
...
```{r}
peak_list <- gcims_select_rois(dir_in = aligned,
                                  dir_out = roispp, 
                                  samples = 1:3,
                                  noise_level = 3)
head(peak_list)
...
We clustered the ROI of dataset using the kmedoids method and the scaled Euclidean distance. Please consult the documentation of function `group_peak_list()` to select other methods and distances. At the end of this process, an R object is obtained (a list):
...
It may happen that ROI belonging to the same cluster are detected more than once in the same sample. The function `gcims_merge_rois()` fuses the repeated ROI into one.

```{r}
merging_res <- gcims_merge_rois(
...
ggplot() +
  geom_rect(
    data = dplyr::filter(merging_res$peak_list_clustered, cluster == "Cluster07"),
    mapping = aes(
...
The function of `gcims_compute_fom()` calculates several of figures of merit of ROI: area, volume, asymmetry and saturation. 
...
peak_list_fom <- gcims_compute_fom(
  dir_in = bsln,
  dir_out = fom,
  peak_list = merging_res$peak_list_clustered,
  cluster_stats = merging_res$cluster_stats
)
head(peak_list_fom)
```
...
You can extract the volume of each sample across samples to create a peak table using the function `build_peak_table()`. This will be useful in case you want perform qualitative and quantitative analysis from these data.
...
To input missing values of the peak table, use the function `gcims_imputate_missing()`. It computes the volume in the region of the reference ROI for each cluster for each sample having a missing value for the cluster.
```{r}
peak_table_imputed <- gcims_impute_missing(
