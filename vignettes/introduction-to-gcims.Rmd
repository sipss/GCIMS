...
#gcims_read_mea(dir_in = wd, dir_out = data)
# For .csv files use:
gcims_read_samples(dir_in = wd, dir_out = data, sftwr = 1)
```
...
samples <- 1:24
ref_number <- NULL
gcims_create_reference(dir_in = reshaped, dir_out = reshaped, samples, ref_number)
```
...
rt_range <- c(0, 850)
dt_range <- c(7, 14)
gcims_cut_samples(dir_in = reshaped, dir_out = cut, samples, rt_range, dt_range)
```
...
samples <- 1:24
time <- "Drift"
gcims_interpolate(dir_in = cut, dir_out = interpd, samples, time)
```
...
time <- "Drift"
samples <- 1:24
gcims_smoothing(dir_in = interpd, dir_out = filtd, samples, time)
```
...
time <- "Drift"
samples <- 1:24
gcims_baseline_removal(dir_in = filtd, dir_out = bslnd, samples, time)
```
...
gcims_alignment(dir_in = bslnd, dir_out = alignd, samples, time, seg_vector, slack_vector)
```
...
samples <- 1:24
time <- "Retention"
gcims_interpolate(dir_in = alignd, dir_out = interpr, samples, time)
```
...
time <- "Retention"
samples <- 1:24
gcims_smoothing(dir_in = interpr, dir_out = filtr, samples, time)
```
...
samples <- 1:24
time <- "Retention"
gcims_alignment(dir_in = bslnr, dir_out = alignr, samples, time)
```
...
dt_range <- NULL
dir_in <- alignr
gcims_plot_spec(dir_in = alignr, sample_num, rt_value, dt_range, colorby = "Name")
```
...
dt_value <- NULL
rt_range <- NULL
gcims_plot_chrom(dir_in = alignr, sample_num, dt_value, rt_range, colorby = "Name")
```
...
```{r}
# ROIs DETECTION
samples <- c(3, 8, 13, 5, 6, 9)
noise_level <- 3
gcims_rois_selection(dir_in = alignr, dir_out = roispp, samples, noise_level)
```
