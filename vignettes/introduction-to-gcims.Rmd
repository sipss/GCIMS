...
The first step is to create all directories and folders where samples will be located and stored along all the process. 
...
All the samples of the working directory are going to be read and converted to the format needed for working with this package.
...
Visualize data is useful to understand data and their changes due to the pre-processing stage. The packages provides three main data visualization functions to perform this task: `gcims_view_sample()`, `gcims_plot_spec()`, and `gcims_plot_chrom()`. By the moment, we will use the function `gcims_view_sample()` to inspect one sample. By doing this, we will figure out the region in which chemical information is concentrated in such a sample.

```{r}
gcims_view_sample(data, sample_num = 1, rt_range = NULL, dt_range = NULL, transform = TRUE)
```
...
The function gcims_cut_samples()` cuts all samples in a rectangle selected by the user. The sample cut can be performed at the beginning, but also at the end after all the pre-processing.
...
```{r}
gcims_view_sample(cut, sample_num = 1, rt_range = NULL, dt_range = NULL, transform = TRUE)
```
...
making very complicated working with them. Take into account that `gcims_cut_samples()` and `gcims_reshape_samples()` perform similar tasks. It can be the case that after cutting all samples (like in this examples) no further reshape is needed. Though, we include the reshape stage of data pre-processing workflow to exemplify how the function `gcims_reshape_samples()` works:
...
GCIMS data usually suffers from noisy, drift and retention time axes misalignment and baseline problems.  Data pre-processing corrects this non-indelilties of data. Use the functions `gcims_prepare_data()`, `gcims_align_data()`, and `gcims_remove_baseline()`to solve each of theses problems, respectively.
...
Data is aligned in drift and retention time axes by applying the function `gcims_alingn_data()`. The correction in drift time axis is multiplicative, that is a multiplicative correction factor is computed for each of the samples so that the position of the Reactant Ion Peak (RIP) in spectra is the same across samples. The correction in retention time axis is achieved using the Parametric Time Warping (PTW) algorithm. 
...
Let'show  gcims data before and after sample alignment. To show particular spectrum/chromatogram for different samples use the functions 'gcims_plot_spec()' and 'gcims_plot_chrom()', respectively.


Compare the spectra of all samples at retention time rt_value = 90 s:
```{r}
gcims_plot_spec(prepared, samples = 1:3, rt_value = 90, dt_range = c(7.2,10), colorby = "Name")
gcims_plot_spec(aligned, samples = 1:3, rt_value = 90, dt_range = c(7.2,10), colorby = "Name")
...
Finally, we do the same, but showing the chromatoagrams of all samples at drift time dt_value = 11.1 ms.
```{r}
# VISUALIZE THE EFFECT OF ALIGNMENT (TOTAL ION CHROMATOGRAM)
gcims_plot_chrom(prepared, samples = 1:3, dt_value = 11.1, rt_range =  NULL, colorby = "Name")
gcims_plot_chrom(aligned, samples = 1:3, dt_value = 11.1, rt_range =  NULL, colorby = "Name")
```
The resulting chromatograms become aligned, as expected. After this pre-processing stage, regions with peaks in data will be detected. Since several peaks may be convoluted in these regions we will call them Regions Of Interest (ROIs).
...
```{r}
# VISUALIZE THE EFFECT OF Baseline Removal
gcims_view_sample(cut, sample_num = 1, rt_range = NULL, dt_range = NULL, transform = TRUE)
gcims_view_sample(bsln, sample_num = 1, rt_range = NULL, dt_range = NULL, transform = TRUE)
```
