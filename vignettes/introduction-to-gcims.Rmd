...
peak_clustering <- clusterPeaks(
  ...
  clustering = list(method = "kmedoids", Nclusters = 50)
)
```
...
plt <- plotRaw(ket2)
plt <- add_peaklist_rect(plt, dplyr::filter(peak_list_clustered, cluster < 10), color_by = "SampleID") + 
...
plt <- add_peaklist_rect(
  plt, 
  dplyr::filter(peak_clustering$peak_list_clustered, cluster < 10),
  color_by = "SampleID",
