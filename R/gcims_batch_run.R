...
#' @param batch_params Argument so we can pass parameters to `gcims_batch_run`.
#'  Ignored for now
#'
#' @return `NULL`. This function is called by its side effects
#' @export
#'
#' @examples
...
#'
#'
gcims_batch_run <- function(dir_in, dir_out, what, ..., batch_params = NULL) {
  files <- list.files(path = dir_in, pattern = "\\.rds$", recursive = TRUE)
