...
rt_alignment <- function(bslnr,aigntr,aux,NumOfSamples,NumOfRows,inicio,fin,max_tr){
  ...
  # Once the positions of the reference peaks are found for each sample, they are aligned taking as patron the position of the reference peaks of the first sample (M1).
  setwd(bslnr)
  M1 <- readMat("Mbr1.mat")[[1]]
  Mr <- M1[1:max_tr, ]
  setwd(aligntr)
  saveRDS(Mr, file = "Mar1.rds")
  tr_sp <- c(1:max_tr)
  ...
  for (i in (2:NumOfSamples)){
    ...
    setwd(aligntr)
    saveRDS(Mr, file = paste0("Mar", i, ".rds"))
  }
