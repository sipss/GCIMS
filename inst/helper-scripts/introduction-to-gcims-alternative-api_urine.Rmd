...
register(SnowParam(workers = 4, progressbar = show_progress_bar, exportglobals = FALSE), default = TRUE)
```
...
samples_directory <- "/home/sergio/sync_ibec/Escritorio/work/ibec/src/GCIMS-Samples/2022-orinas/"
```
...
```{r message=FALSE}
annotations <- readxl::read_excel(file.path(samples_directory, "Metadata.xlsx"))
annotations
```
...
dataset <- GCIMSDataset(
  ...
  on_ram = FALSE # You probably should set this to FALSE if you have more 
...
```{r}
ket1 <- getGCIMSSample(dataset, sample = "S402_Cancer_M")
```
...
```
```{r}
plotRIC(dataset, rt_range = c(0, 250))
```
...
```{r}
ket1afterfilter <- getGCIMSSample(dataset, sample = "S402_Cancer_M")
ket1afterfilter
```
...
ket1 <- getGCIMSSample(dataset, "S402_Cancer_M")
ket1 <- findPeaks(
  ket1,
  noise_level = 3,
  rt_length_s = 3,
...
plt <- plotRaw(ket1, dt_range = c(7, 15), rt_range = c(0, 300))
...
```{r}
ket2 <- getGCIMSSample(dataset, "S246_Control_F")
```
...
```{r}
plt <- plotRaw(ket2, dt_range = c(7, 12), rt_range = c(0, 250))
plt <- add_peaklist_rect(plt, peaks(dataset), color_by = "SampleID")
plt
```
...
```{r}
plt <- plotRaw(ket2, dt_range = c(7, 12), rt_range = c(0, 250))
plt <- add_peaklist_rect(plt, peaks(dataset))
plt + facet_wrap(~SampleID)
```
...
peak_clustering <- clusterPeaks(
  ...
  clustering = list(method = "kmedoids", Nclusters = 150)
)
```
...
plt <- plotRaw(ket2, dt_range = c(7, 12), rt_range = c(0, 200))
...
```{r fig.height=10, fig.width=10}
plt <- plotRaw(ket2, dt_range = c(7, 12), rt_range = c(0, 200))
plt <- add_peaklist_rect(plt, peak_list_clustered, color_by = "cluster") + 
  facet_wrap(~SampleID)
...
ggplot(cosa2) + geom_boxplot(aes(x = gsub("/.*", "", SampleID), y = sil_width))
```
...
ket1 <- getGCIMSSample(dataset, "S246_Control_F")
```
